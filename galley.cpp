...
void Galley::render(float leftOarsAngle, float rightOarsAngle, glm::mat4 camera, glm::vec4 vector) {
	...
	galley.draw(P, camera, galleyMatrix, vector);
	...
	for (int i = 0; i < 7; i++) {
		...
		oar.draw(P, camera, rotatedRightOarMatrix, vector);
		oar.draw(P, camera, rotatedLeftOarMatrix, vector);
	}
}
